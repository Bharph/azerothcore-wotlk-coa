...
-- Scope and safety:
--   * Only `character_inventory`.`bag` and `character_inventory`.`slot` are rewritten. No row is inserted or
--     deleted, no stack is merged or split, and `item_instance` is untouched, so no item can be lost or
--     duplicated here.
--   * Stacks in backpack slots 23..38 and inside the four equipped bag containers (19..22) are relocated;
--     the containers themselves are not moved. Bank slots 39..66 and bank bags 67..73 are deliberately left
--     alone: those load fine either way, and silently emptying a player's bank is worse than a currency tab
--     that only counts what is carried.
